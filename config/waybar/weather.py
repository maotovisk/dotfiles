...
data["tooltip"] = (
    f"<b>{weather['current_condition'][0]['lang_pt'][0]['value']} <span foreground='{COLOR_CITY_TEMP}'>{weather['current_condition'][0]['temp_C']}°</span></b>\n"
)
data["tooltip"] += (
    f"Sensação térmica: <span foreground='{COLOR_FEELS_LIKE}'>{weather['current_condition'][0]['FeelsLikeC']}°</span>\n"
)
data["tooltip"] += (
    f"Vento: <span foreground='{COLOR_WIND}'>{weather['current_condition'][0]['windspeedKmph']}Km/h</span>\n"
)
data["tooltip"] += (
    f"Umidade: <span foreground='{COLOR_HUMIDITY}'>{weather['current_condition'][0]['humidity']}%</span>\n"
)
...
for i, day in enumerate(weather["weather"]):
    ...
    data["tooltip"] += (
        f"<span foreground='{COLOR_MAX_TEMP}'>\uf062 {day['maxtempC']}°</span> <span foreground='{COLOR_MIN_TEMP}'>\uf063 {day['mintempC']}°</span> "
    )
    data["tooltip"] += (
        f"<span foreground='{COLOR_SUNRISE}'>\uf185</span> {day['astronomy'][0]['sunrise']} <span foreground='{COLOR_SUNSET}'>\uf186</span> {day['astronomy'][0]['sunset']}\n"
    )

    # Table header for hourly forecast
    data["tooltip"] += (
        "<span font_family='monospace'><b>Hora  Icon  Térmica  Descrição                   Chances</b>\n"
    )
    data["tooltip"] += (
        "----- ----- ------- ---------------------------- --------------------</span>\n"
    )
